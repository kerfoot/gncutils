...
def build_dba_yo(dba_reader):

    sensor_names = [s['sensor_name'] for s in dba_reader['sensors']]

    # Find llat_time and llat_pressure
    if 'llat_time' not in sensor_names:
        logger.warning('Parsed dba does not contain llat_time: {:s}'.format(dba_reader['file_metadata']['source_file']))
        return
    if 'llat_pressure' not in sensor_names:
        logger.warning('Parsed dba does not contain llat_pressure: {:s}'.format(dba_reader['file_metadata']['source_file']))
        return

    ti = sensor_names.index('llat_time')
    pi = sensor_names.index('llat_pressure')

    return dba_reader['data'][:, [ti, pi]]


# def build_yo(data):
#
#     # Initialize yo array
#     yo = np.empty((len(data),2)) * np.nan
#
#     row_count = -1
#     for row in data:
#
#         row_count += 1
#
#         if 'drv_timestamp' in row:
#             yo[row_count,TIME_DIM] = row['drv_timestamp']
#
#         if 'drv_depth' in row:
#             yo[row_count,DATA_DIM] = row['drv_depth']
#
#     return yo
